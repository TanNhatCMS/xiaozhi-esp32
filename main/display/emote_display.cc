...
#define ICON_LISTEN "listen"
    ...
    void EmoteDisplay::SetMusicInfo(const char *song_name)
    {
        if (!engine_)
        {
            ESP_LOGW(TAG, "SetMusicInfo: engine not initialized");
            return;
        }
        ...
        {
            // Use a std::string to ensure a stable c_str() while the label API copies/clones the text.
            std::string music_text(song_name);
            gfx_label_set_text(g_obj_label_toast, music_text.c_str());
        }
        else
        {
            // Clear the displayed song name
            gfx_label_set_text(g_obj_label_toast, "");
        }
