...
void McpServer::AddCommonTools()
{
    ...
    if (music)
    {
        AddTool("self.music.play_song",
                "Phát một bài hát cụ thể. Dùng tool này khi người dùng yêu cầu mở nhạc. "
                "Hệ thống sẽ tự lấy thông tin bài hát và bắt đầu stream phát.\n"
                "Tham số:\n"
                "  `song_name`: Tên bài hát cần phát (bắt buộc).\n"
                "  `artist_name`: Tên ca sĩ / nghệ sĩ của bài hát (tùy chọn, mặc định là chuỗi rỗng).\n"
                "Giá trị trả về:\n"
                "  Trạng thái phát nhạc. Không cần xác nhận lại — bài hát sẽ phát ngay.",
                PropertyList({
                    Property("song_name", kPropertyTypeString),      // Tên bài hát (bắt buộc)
                    Property("artist_name", kPropertyTypeString, "") // Tên nghệ sĩ (tùy chọn, mặc định "")
                }),
                [music](const PropertyList &properties) -> ReturnValue
                {
                    ...
                    {
                        return "{\"success\": false, \"message\": \"Không thể lấy nguồn nhạc\"}";
                    }
                    auto download_result = music->GetDownloadResult();
                    ESP_LOGI(TAG, "Kết quả chi tiết bài hát: %s", download_result.c_str());
                    return "{\"success\": true, \"message\": \"Đang phát nhạc\"}";
                });

        AddTool("self.music.set_display_mode",
                "Thiết lập chế độ hiển thị khi phát nhạc. Có thể chọn hiển thị phổ tần (spectrum) "
                "hoặc hiển thị lời bài hát (lyrics). Ví dụ: người dùng nói 'bật phổ tần', 'hiển thị phổ tần', "
                "'bật lời bài hát', 'hiển thị lời bài hát' thì sẽ chuyển sang chế độ tương ứng.\n"
                "Tham số:\n"
                "  `mode`: Chế độ hiển thị. Giá trị hợp lệ: 'spectrum' (hiển thị phổ tần) hoặc 'lyrics' (hiển thị lời).\n"
                "Giá trị trả về:\n"
                "  Kết quả cập nhật chế độ hiển thị.",
                PropertyList({
                ...
                [music](const PropertyList &properties) -> ReturnValue
                {
                    ...
                    {
                        return "{\"success\": false, \"message\": \"Chế độ hiển thị không hợp lệ. Vui lòng dùng 'spectrum' hoặc 'lyrics'.\"}";
                    }

                    return "{\"success\": false, \"message\": \"Không thể đặt chế độ hiển thị\"}";
                });
    }
